...
namespace Toy {
	...
	{
		TOY_CORE_ERROR("GLFW Error ({0}):{1}", error, description);
	}
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		TOY_CORE_INFO("Createing window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			TOY_CORE_ASSERT(success, "Could not initialize GLFW!");
